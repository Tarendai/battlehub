...
import org.darkstars.battlehub.gui.CPlayerDataWindow;
...
public class CBattleList extends CView {
    ...
    private CBattleTableModel jbm;
    private TableRowSorter<TableModel> ts = null;
    private CMyRowFilter filter = null;
...
//       }
        ...
        public boolean include(Entry entry) {
            ...
            if (bi.IsIngame() && (!ingame)) {
                ...
                return false;
            } else if ((bi.maxplayers == bi.GetPlayers().size()) && (!full)) {
                return false;
            } else {
    ...
    public void NewEvent(final CEvent e) {
        if (e.IsEvent(CEventTypes.ADDEDBATTLE)) {
            int a = 5;
            Runnable doWorkRunnable2 = new Runnable() {

                @Override
                public void run() {
                    jbm.AddBattle((CBattleInfo) e.object);
            ...
            Iterator<IBattleInfo> i = LMain.protocol.GetBattles().values().iterator();
            while (i.hasNext()) {
                CBattleInfo info = (CBattleInfo) i.next();
                if (info.GetHost().name.equalsIgnoreCase(e.data[1])) {
                    ...
                    if (Misc.getInGameFromStatus(cstatus) == 1) {
                        ...
                        if (notifyOnBattleFinish.contains(e.data[1])) {
                            ...
                            LMain.Toasts.AddMessage("The battle hosted by " + e.data[1] + " has just finished");
                        }
                    }
                    if (jbm != null) {
                        Runnable doWorkRunnable = new Runnable() {
    ...
    private void JoinBattle() {
        ...
        if (bi == null) {
            LMain.Toasts.AddMessage("please select a battle to join");
            return;
        }
        JoinBattle(bi);
    }
    ...
    private void JoinBattle(CBattleInfo bi) {
        if (!bi.isJoinable()) {
            //
            LMain.Toasts.AddMessage("Sorry this game is not joinable, make sure you have the needed game engine installed");
            return;
        }
        ...
        if (bi.IsIngame() == false) {
            if (bi.getEngine().equals("spring")) {
                if (!CSync.HasMod(bi.GetMod())) {
                    ...
                    if (!CSync.HasMod(bi.GetMod())) {
                        ...
                        LMain.Toasts.AddMessage("You dont have this mod!!");
                        return;
                    }
                }
            }
            ...
            if (bi.natType == 1) {
                if (!CHolePuncher.SendRequest(LMain.protocol.GetUsername())) {
                    LMain.Toasts.AddMessage("Holepunching failed....");
                    return; // holepunching failed we dont start
                }
            }
            LMain.protocol.SendTraffic("JOINBATTLE " + bi.GetID() + " " + String.valueOf(BattlePass.getPassword()));
        } else {
            LMain.Toasts.AddMessage("Game already in progress");
        }
    }
    ...
    private void initComponents() {
        ...
        ShowMenu.setText("null");

        IngameCheckMenuItem.setSelected(true);
        IngameCheckMenuItem.setText("Ingame"); // NOI18N
        ...
        PasswordCheckMenuItem.setText("Passworded"); // NOI18N
        ...
        LockedCheckMenuItem.setText("Locked"); // NOI18N
        ...
        LadderCheckMenuItem.setText("Ladder"); // NOI18N
        ...
        FullCheckMenuItem.setText("Full"); // NOI18N
        ...
        MessageOnOpenCheckMenu.setText("null");
        ...
        HostInfoMenuItem.setText("View Host Info/Send PM"); // NOI18N
        ...
        jBattleTable.setRowHeight(20);
        jBattleTable.setRowMargin(0);
        jBattleTable.setRowSorter(ts);
        jBattleTable.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        ...
        TableColumn col = jBattleTable.getColumnModel().getColumn(6);
        col.setPreferredWidth(32);
        col.setMinWidth(32);
        col.setMaxWidth(32);

        col = jBattleTable.getColumnModel().getColumn(7);
        col.setPreferredWidth(64);
        col.setMinWidth(64);
        col.setMaxWidth(64);

        col = jBattleTable.getColumnModel().getColumn(1);
        col.setPreferredWidth(24);
        col.setMaxWidth(24);

        col = jBattleTable.getColumnModel().getColumn(2);
        col.setPreferredWidth(24);
        col.setMaxWidth(24);

        col = jBattleTable.getColumnModel().getColumn(9);
        col.setPreferredWidth(48);
        col.setMaxWidth(48);

        col = jBattleTable.getColumnModel().getColumn(10);
        col.setPreferredWidth(28);
        col.setMaxWidth(28);

        col = jBattleTable.getColumnModel().getColumn(11);
        ...
        BattlePass.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(255, 255, 0), 1, true));
        ...
        battleCountLabel.setText("0 Battles Listed"); // NOI18N

        SurpriseMeButton.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/UI/dice.png"))); // NOI18N
        SurpriseMeButton.setText("Join Random Battle"); // NOI18N
    ...
    private void MessageOnOpenCheckMenuMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_MessageOnOpenCheckMenuMouseReleased
        ...
        if (MessageOnOpenCheckMenu.isSelected()) {
            if (notifyOnBattleFinish.contains(i.GetHost().name) == false) {
                notifyOnBattleFinish.add(i.GetHost().name);
            }
        } else {
    ...
    private void SurpriseMeButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SurpriseMeButtonActionPerformed
        ...
        if (!randomBattleList.isEmpty()) {
            ...
            LMain.Toasts.AddMessage("No suitable battles found");
        }
    ...
    private void HostInfoMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_HostInfoMenuItemActionPerformed
        ...
        CBattleTableModel m = (CBattleTableModel) jBattleTable.getModel();
        CBattleInfo i = m.GetBattleAt(row);
        CPlayerDataWindow p = new CPlayerDataWindow(LM, i.GetHost());
        p.setVisible(true);
