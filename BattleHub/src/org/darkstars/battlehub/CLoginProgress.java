...
public class CLoginProgress extends CView {
    ...
    public CLoginProgress(LMain L) {
        ...
        if (Boolean.valueOf(CUserSettings.GetValue("autologin", "false"))) {
            ...
            if (protocol.equalsIgnoreCase("TASServer v0.33")) {
                ...
                LMain.Toasts.AddMessage(java.util.ResourceBundle.getBundle("aflobby/languages").getString("CLoginPanel.unsupported_protocol"));
                return;
            }
    ...
    public void NewEvent(CEvent e) {
        if (e.IsEvent("LOGININFOEND") == true) {
            ...
            //LMain.protocol.SendTraffic("SAYPRIVATE []Avalon BattleHub " + CUpdateChecker.version + " reporting");
            //LM.connection.SendLine ("JOIN aflobby AFL");
            LMain.protocol.JoinChannel("battlehub", null);
            LMain.protocol.SendTraffic("SAYEX battlehub is running BattleHub " + CUpdateHandler.version);
            LM.RemoveView(this);
        } else if (e.IsEvent(CEventTypes.CONNECTED)) {
            Runnable doWorkRunnable = new Runnable() {
